# MiniMax-H3 任意图片 + 参考视频工作流

## 产品定义

用户上传一张参考图和一段参考视频，系统把参考图作为 `Mixed 1 / reference_image`，把参考视频作为 `Mixed 2 / reference_video`，调用 Metaso MiniMax-H3 生成新视频。

这套工作流默认采用**保留原背景模式**：保留原视频的场景、道具、地面、墙面、光影、遮挡关系和镜头运动，只替换人物或主体。白棚模式只适合纯动作复刻、产品展示、T 台、教学动作，不适合强场景叙事视频。
